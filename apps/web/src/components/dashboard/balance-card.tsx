  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginTop: 24, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
        {[
          { label: t('consumed'), value: `¥${totalSpent.toFixed(2)}`, color: '#f472b6' },
          { label: t('totalRecharged'), value: `¥${totalRecharged.toFixed(2)}`, color: '#34d399' },
          { label: t('balanceStatus'), value: available > 0 ? `✅ ${t('balanceOk')}` : `⚠️ ${t('balanceLow')}`, color: available > 0 ? '#34d399' : '#f59e0b' },
        ].map((s) => (
          <div key={s.label}>
